    ...
    <div className={styles.specimen}>
      <div className={styles.header}>
        <div className={styles.nameRow}>
          <span className={`type-caption-md ${styles.nameTag}`}>{token.name}</span>
          ...
          {token.fixedTokens.length > 0 && (
            <div className={styles.tokenGroup}>
              ...
              {token.fixedTokens.map((v) => (
                <code key={v} className={`type-caption-sm ${styles.tokenVar}`}>
                  {v}
                </code>
              ))}
            </div>
          ...
          {token.contextualTokens.length > 0 && (
            <div className={styles.tokenGroup}>
              ...
              {token.contextualTokens.map((v) => (
                <code key={v} className={`type-caption-sm ${styles.tokenVarContextual}`}>
                  {v}
                </code>
              ))}
            </div>
